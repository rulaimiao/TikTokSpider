...
class TikTokSpider(object):
    ...
    def crawl(self):
        ...
        :return: None
        '''
        _id = self.get_douyin_id()
        if not self.is_valid_id(_id):
            return

        username, dytk = self.get_name_and_dytk(_id)
        if not (username and dytk):
            return

        self.makedir(username)
        VIDEO_URLS = self.get_all_video_urls(_id, 0, dytk)
        for index, item in enumerate(VIDEO_URLS, 1):
            name = item[0]
            if name == '抖音-原创音乐短视频社区':
                name = name + str(index)
            url = item[1]
            self.download_video(index, username, name, url)
            sleep(1)

    def handle_headers_and_url(self, headers, url, user_id, max_cursor, dytk):
        import re
    ...
    def get_all_video_urls(self, user_id, max_cursor, dytk):
        '''
        获取用户所有视频的源地址url
        ...
        headers, url = self.handle_headers_and_url(URL_HEADERS, URL, user_id, max_cursor, dytk)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for li in data['aweme_list']:
                    ...
                    self.VIDEO_URLS.append([name, url])
                ...
                if data['has_more'] is True and data.get('max_cursor') != 0:
                    ...
                    print('正在收集第%s页视频地址' % (self.PAGE))
                    self.PAGE += 1
                    return self.get_all_video_urls(
    ...
    def download_video(self, index, username, name, url, retry=3):
        '''
        下载视频,显示进度
        ...
        try:
            ...
            # 失败重试3次，超过放弃
                ...
                retry -= 1
                self.download_video(index, username, name, url, retry)
            else:
                return
        except Exception as e:
    ...
    def get_name_and_dytk(self, num):
        '''
        获取用户名和dytk
    ...
    def get_douyin_id(self):
        '''
        获取抖音用户id

        :return:    user_id
        '''
        _id1 = self.get_id_from_cmd(sys.argv[1:])
        if _id1:
            return _id1

        _id2 = self.get_id_from_input()
        return _id2 if _id2 else None

    def get_id_from_cmd(self, cmd_args):
        '''
        从命令行获取user_id
...
if __name__ == '__main__':
    douyin = TikTokSpider(URL_HEADERS, URL)
    douyin.crawl()
